...
public class StoryRecorder implements NotificationCenter.NotificationCenterDelegate {
    ...
    public static class SourceView {
        ...
        public static SourceView fromAvatarImage(IAvatarView avatarImage, boolean isForum) {
            if (avatarImage == null || avatarImage.getRootView() == null) {
                return null;
            }
            float scale = ((View) ((View) avatarImage).getParent()).getScaleX();
            ...
            SourceView src = new SourceView() {
                @Override
                protected void show(boolean sent) {
                    avatarImage.showView();
                }

                @Override
                protected void hide() {
                    avatarImage.hideView();
                }
            };
            final int[] loc = new int[2];
            final float[] locPositon = new float[2];
            avatarImage.getRootView().getLocationOnScreen(loc);
            AndroidUtilities.getViewPositionInParent((View) avatarImage, (ViewGroup) avatarImage.getRootView(), locPositon);
